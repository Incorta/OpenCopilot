...
from operators.incorta.api_helpers import connect_jdbc
...
def get_commands_help():
    return {
        "overview": """ Creates a sql query and executes it on Incorta system to retrieve data from the database. """
        ,
        "commands": [
            {
                "command": {
                ...
                "command_description": "Creates a SQL query to be executed on Incorta System. You must first list all possible column names in column_names_list, then choose from this list column names to be used in the query. Never use a column name in the query other than column names in column_names_list."
            }
        ]
    }
...
def handle_command(command):
    if command["command_name"] == Commands.getQuery.name:
        return connect_jdbc.run_sql_query_jdbc(command["args"]["query"])
    else:
