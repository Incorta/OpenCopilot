...
from service.controller.session_handler import QueryState
from service.controller.tasks import TaskStatus
from service.utils.exceptions import InapplicableQuestionException
...
def summarize_session_queries(user_session, max_history_size=service_utils_constant_module.HISTORY_LIMIT):
    ...
    for query in user_session.queries_list[:-1]:
        ...
        if state not in (QueryState.SUCCESS, QueryState.EXCEPTION):  # TODO clean it to use the QueryState ENUM
            continue
        ...
        if tasks:
            ...
            elif state == QueryState.EXCEPTION:
                for task in tasks:
                    if task['status'] == TaskStatus.ERROR and "error" in task and isinstance(query.exception, InapplicableQuestionException):
                            summary_queue.append({
                                "user_query_msg": query_msg,
                                "reply": {"reply": query.exception.message}
                            })
